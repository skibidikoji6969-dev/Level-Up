...
import type { DraftSlot } from './planner';
import { findNextAvailableWindow, timeToMinutes, minutesToTime } from './planner';
...
// ---------------- Smart Slot Planner (Sprint 2) ----------------

export async function updateEnergyProfile(patch: {
...
export async function createPlannerSlot(input: {
  date: string;
  startTime: string;
  endTime: string;
  ...
  await db.plannerSlots.add(slot);
  return slot;
}

/** Bulk-inserts an accepted "Generate Today's Plan" preview in one go. */
export async function savePlannerDrafts(drafts: DraftSlot[]): Promise<PlannerSlot[]> {
  const now = nowISO();
  const slots: PlannerSlot[] = drafts.map((d) => ({
    id: uid(),
    date: d.date,
    ...
    energyRequirement: d.energyRequirement,
    status: 'planned',
    completionPct: 0,
    notes: d.notes,
    autoGenerated: true,
  ...
  if (slots.length) await db.plannerSlots.bulkAdd(slots);
  return slots;
}

export async function updatePlannerSlot(id: string, patch: Partial<PlannerSlot>) {
  await db.plannerSlots.update(id, patch);
}
...
/**
 * Marks a slot done and routes XP through the SAME actions Sprint 1 already
 * uses — no parallel XP system. New Learning/Practice log a real
 * StudySession (which also updates the Topic's timeSpentMin/questionsSolved
 * and awards 'study_session' XP). Revision reuses logTopicRevision (awards
 * 'revision' XP, bumps revisionCount). Mock Test reuses logMockTest.
 * Assignment/Other award 'task_complete' XP directly, same as finishing a
 * planner Task elsewhere in the app.
 */
export async function completePlannerSlot(id: string, actualMinutes?: number) {
  const slot = await db.plannerSlots.get(id);
  if (!slot) return;

  const duration = actualMinutes ?? slot.estimatedDuration;

  if ((slot.taskType === 'new_learning' || slot.taskType === 'practice') && slot.subjectId) {
  ...
  } else if (slot.taskType === 'revision' && slot.topicId) {
    await logTopicRevision(slot.topicId);
  } else if (slot.taskType === 'mock_test') {
    await logMockTest(slot.subjectId ?? undefined);
  } else {
    await awardXP('task_complete', undefined, slot.notes || 'Planner slot completed');
  }

  await db.plannerSlots.update(id, { status: 'completed', completionPct: 100 });
}

export type RescheduleTarget = 'tonight' | 'tomorrow' | 'next_available' | 'manual';

/**
 * Handles a missed slot per Fix spec: reschedule tonight/tomorrow/next
 * available slot, or hand back to the caller for manual placement. The
 * original slot is marked 'rescheduled' (not deleted) and a fresh slot is
 * created linking back via rescheduledFromId — nothing here touches
 * discipline.ts or tasksMissed, so a miss doesn't punish the discipline
 * score or break a streak.
 */
export async function rescheduleMissedSlot(
  id: string,
  target: RescheduleTarget,
  manual?: { date: string; startTime: string; endTime: string }
) {
  const slot = await db.plannerSlots.get(id);
  if (!slot) return null;

  const allSlots = await db.plannerSlots.toArray();
  const durationMin = timeToMinutes(slot.endTime) - timeToMinutes(slot.startTime);

  let target_: { date: string; startTime: string; endTime: string } | null = null;

  if (target === 'manual' && manual) {
    target_ = manual;
  } else if (target === 'tonight') {
    target_ = { date: slot.date, startTime: '20:00', endTime: minutesToTime(timeToMinutes('20:00') + durationMin) };
  } else if (target === 'tomorrow') {
    const tomorrow = new Date(slot.date);
    tomorrow.setDate(tomorrow.getDate() + 1);
    const tomorrowISO = tomorrow.toISOString().slice(0, 10);
    target_ = { date: tomorrowISO, startTime: slot.startTime, endTime: slot.endTime };
  } else {
    const found = findNextAvailableWindow(allSlots, durationMin, slot.date, slot.id);
  ...
  await db.plannerSlots.update(id, { status: 'rescheduled' });
  const newSlot = await createPlannerSlot({
    date: target_.date,
    startTime: target_.startTime,
    endTime: target_.endTime,
    subjectId: slot.subjectId,
    topicId: slot.topicId,
    taskType: slot.taskType,
    priority: slot.priority,
    estimatedDuration: durationMin,
    energyRequirement: slot.energyRequirement,
    notes: slot.notes,
    autoGenerated: slot.autoGenerated,
    rescheduledFromId: slot.id,
  });
  return newSlot;
}
